...
def format_blog_body_html(raw):
    ...
    reads identically to the JS-rendered blog-post.html version: plain text
    (blank-line-separated) becomes real <p>/<ul>/<ol>/<h3> markup, with
    pull-quotes for short rhetorical questions and a lead-paragraph class
...
def generate_blog_pages(posts):
    ...
    for post in published:
        ...
        summary = post.get('short_summary') or 'Health information and medical insights from Ibn Sina Hospital.'
        category = post.get('category') or post.get('department') or 'Health & Wellness'
        image = post.get('cover_image_url') or 'https://i.ibb.co/NgNyCQgf/8e1694fa3791.webp'
        published_at = post.get('published_at', '')
        formatted_date = format_blog_date(published_at)
        ...
        if formatted_date:
            date_meta_html = f"""
                    <span class="blog-meta-item">
                      ...
                      <time datetime="{html.escape(published_at)}">{html.escape(formatted_date)}</time>
                    </span>
        ...
        <div class="container">
          <article class="premium-blog-post">
            ...
            <header class="blog-article-hero">
              <div class="blog-article-category">{category_esc}</div>
              <h1 class="blog-article-title">{title_esc}</h1>
              <p class="blog-article-summary">{summary_esc}</p>
              <div class="blog-article-meta">{date_meta_html}
                <span class="blog-meta-item">
                  <span aria-hidden="true">⏱</span>
